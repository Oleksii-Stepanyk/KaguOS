...
function cpu_exec {
    ...
    case "${REG_OP_VAL}" in
        ...
        ${OP_RENDER_BITMAP})
            ...
            ;;
        ${OP_ENCRYPT_DATA})
            local DATA="${REG_A_VAL}"
            if [[ "$DATA" == "" ]]; then
                write_to_address $REG_RES ""
                return
            fi
            local DATA_SIZE="${#DATA}"
            local SHIFTED_DATA=$(echo "$DATA" | tr 'b-ds-tB-DST' 'c-dbtsC-DBTS')
            write_to_address $REG_RES "${SHIFTED_DATA:DATA_SIZE-SHIFT:1}${SHIFTED_DATA:0:DATA_SIZE-SHIFT}"
            ;;
        ${OP_DECRYPT_DATA})
            local DATA="${REG_A_VAL}"
            if [[ "$DATA" == "" ]]; then
            ...
            local DATA_SIZE="${#DATA}"
            local DECRYPTED_DATA=$(echo "${DATA:1:DATA_SIZE-SHIFT}${DATA:0:1}" | tr 'c-dbtsC-DBTS' 'b-ds-tB-DST')
