...
# To encrypt data place the data in REG_A
# Set REG_OP to OP_ENCRYPT_DATA and call INSTR_CPU_EXEC.
# Data encrypted in two steps: 1 step is specific shifting (b->c, c->d, d->b, s->t, t->s, same with capital letters)
# 2 step is doing right one-letter shift (hello -> ohell)
export OP_ENCRYPT_DATA=25

# To decrypt data place the data in REG_A
# Set REG_OP to OP_DECRYPT_DATA and call INSTR_CPU_EXEC.
# Data decrypted in two steps: 1 step is specific shifting (b->d, c->b, d->c, s->t, t->s, same with capital letters)
# 2 step is doing left one-letter shift (ohell -> hello)
export OP_DECRYPT_DATA=26

# OP codes from 27 to 28 are reserved for future use.
